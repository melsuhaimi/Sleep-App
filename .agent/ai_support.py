...
def git(repo: Path, *args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    if check and proc.returncode:
        fail("GIT_ERROR", f"git {' '.join(args)} failed", returncode=proc.returncode, stderr=proc.stderr.strip())
    return proc
...
def restore_clean(repo: Path) -> None:
    ...
    for rel in sorted(untracked, key=lambda v: v.count("/"), reverse=True):
...
def write_json(path: str | Path | None, value: dict[str, Any], repo: Path) -> None:
    if path is None:
        return
    target = Path(path)
    write_json_atomic(target if target.is_absolute() else repo / target, value)
...
def _context(repo: Path, rel: str, limit: int) -> dict[str, Any]:
    ...
    if size > limit:
        fail("CONTEXT_OVERFLOW", "task context file exceeds configured AI context limit", path=rel, size=size, limit=limit)
    return {"path": rel, "exists": True, "sha256": file_sha256(path), "content": path.read_text(encoding="utf-8")}
...
def build_prompt(manifest: dict[str, Any], *, start=None) -> str:
    ...
    total = sum(len(item["content"].encode()) for item in context)
    if total > total_limit:
        fail("CONTEXT_OVERFLOW", "AI context exceeds configured total limit", size=total, limit=total_limit)
    payload = {
        "run_id": manifest["run_id"], "manifest_commit": manifest["repo"]["commit"],
        "task": manifest["task"], "policy": manifest["policy"], "context": context,
    }
    rules = (
