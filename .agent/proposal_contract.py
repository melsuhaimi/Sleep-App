"""Validation and mutation boundary for externally authored engineering proposals."""
...
from proposal_support import changed, file_fingerprint, git
...
def _string_list(value: Any, field: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(v, str) for v in value):
        fail("PROPOSAL_INVALID", f"{field} must be an array of strings")
    return list(value)
...
def validate_proposal(
    ...
    if not isinstance(proposal, dict):
        fail("PROPOSAL_INVALID", "proposal root must be an object")
    if set(proposal) != REQUIRED_KEYS:
        fail(
            "PROPOSAL_INVALID",
            "proposal fields do not match the result contract",
            missing=sorted(REQUIRED_KEYS - set(proposal)),
            extra=sorted(set(proposal) - REQUIRED_KEYS),
        )
    if proposal.get("schema_version") != 1:
        fail("PROPOSAL_INVALID", "schema_version must equal 1")
    ...
    if status not in ALL_STATUSES:
        fail("PROPOSAL_INVALID", "proposal status is not recognized", status=status)
    if not isinstance(proposal.get("summary"), str):
        fail("PROPOSAL_INVALID", "summary must be a string")
    ...
    if not isinstance(edits, list):
        fail("PROPOSAL_INVALID", "edits must be an array")
    normalized_edits: list[dict[str, str]] = []
    for edit in edits:
        if not isinstance(edit, dict) or set(edit) != EDIT_KEYS:
            fail("PROPOSAL_INVALID", "each edit must contain only path and content")
        path = edit.get("path")
        content = edit.get("content")
        if not isinstance(path, str) or not isinstance(content, str):
            fail("PROPOSAL_INVALID", "edit path and content must be strings")
    ...
    per_file = cfg.get("max_proposal_edit_file_bytes", 65536)
    total_limit = cfg.get("max_proposal_total_edit_bytes", 196608)
    if not isinstance(per_file, int) or per_file <= 0 or not isinstance(total_limit, int) or total_limit <= 0:
        fail("INVALID_CONFIG", "proposal edit limits must be positive integers")
    ...
    for item in normalized_edits:
        ...
        if size > per_file:
            fail(
                ...
                "proposal file exceeds configured edit limit",
                path=rel,
                size=size,
                limit=per_file,
            )
    ...
    if total > total_limit:
        fail(
            "EDIT_BUDGET_EXCEEDED",
            "proposal exceeds configured total edit limit",
            size=total,
            limit=total_limit,
        )
    return proposal
...
def apply_proposal(repo: Path, proposal: dict[str, Any]) -> None:
    for edit in proposal["edits"]:
        ...
        fd, temp_name = tempfile.mkstemp(prefix=".proposal-edit-", dir=target.parent)
        try:
...
def assert_expected_mutation(repo: Path, proposal: dict[str, Any]) -> dict[str, str]:
    ...
    if actual != expected:
        fail(
            "UNEXPECTED_MUTATION",
            "worktree mutation set differs from the validated proposal",
            expected=expected,
            actual=actual,
        )
